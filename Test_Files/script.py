...
for i in range(1):
    represent, pathways = separate.ipfmc_average(omic_list[i], BP_data)
    represents.append(np.array(represent))
    #print(represent)
    pathways_list.append(pathways)

represent, pathways = separate.ipfmc_average(omic_list[3], mirBP_data)  # Here processes miRNA dataset
represents.append(np.array(represent))
pathways_list.append(pathways)
...
print(Gene_rank)
miRNA_names = list(omic_list[0].index)  # obtain all gene names occured in the omics dataset
miRNA_rank = analysis.gene_occurrence(Full_Genes=Gene_names, Sel_Pathways=pathways_list[0], Full_Pathways=mirBP_data)
print(miRNA_rank)