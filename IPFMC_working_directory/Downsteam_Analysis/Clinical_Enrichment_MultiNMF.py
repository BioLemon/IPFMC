...
def Get_Right_Label(cancer,combine,k,method):
    ...
    A_Labels_dir = f'{Cluster_dir}/{Cancer_type}/{Method}/{k}{Temp_str}.csv'  

    Temp_str = ''
    for data in combine:
        Temp_str = f'{Temp_str}{data}_'
    B_Labels_dir = f'{Cluster_dir}/{Cancer_type}/{Method}/{Temp_str}{k}.csv'  

    Temp_str = ''
    for data in combine:
        Temp_str = f'{Temp_str}{data}_'
    Temp_str = Temp_str[:-1]
    C_Labels_dir = f'{Cluster_dir}/{Cancer_type}/{Method}/{Temp_str}{k}.csv'  

    Temp_str = ''
    for data in combine:
        Temp_str = f'{Temp_str}{data}_'
    D_Labels_dir = f'{Cluster_dir}/{Cancer_type}/{Method}/{Temp_str}res_{k}.csv'

    Temp_str = ''
    for data in combine:
        Temp_str = f'{Temp_str}{data}_'
    E_Labels_dir = f'{Cluster_dir}/{Cancer_type}/{Method}/dataset #1 Complete_{Cancer_type}_{Temp_str}K{k}_cluster.csv'
    ...
    elif os.path.exists(C_Labels_dir):
        label_file = pd.read_csv(C_Labels_dir)
    elif os.path.exists(D_Labels_dir):
        label_file = pd.read_csv(D_Labels_dir)
    elif os.path.exists(E_Labels_dir):
        label_file = pd.read_csv(E_Labels_dir,header=None)
    else:
        labels = [1]
        return labels
...
for Cancer_type in Cancer_type_list:  
    ...
    for Combine in Data_Combine_list:
        ...
        for k in range(2,9):
            ...
            ps = test_enrichment(Full_Clinical,clinical_param=Clinical_param)
            print(Cancer_type)
            print(ps)
            Num_Sig = 0
            for x in ps:
