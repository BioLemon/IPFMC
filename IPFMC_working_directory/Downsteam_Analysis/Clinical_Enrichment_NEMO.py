...
def Get_Right_Label(cancer,combine,k,method):
    ...
    Temp_str = ''
    if {'cnv'} & set(combine):
        Temp_str = f'{Temp_str}_CNV'
    if {'me'} & set(combine):
        Temp_str = f'{Temp_str}_Methy'
    if {'m'} & set(combine):
        Temp_str = f'{Temp_str}_mRNA'
    if {'mi'} & set(combine):
        Temp_str = f'{Temp_str}_miRNA'
    C_Labels_dir = f'{Cluster_dir}/{Cancer_type}/{Method}/nemo{Temp_str}_{k}.csv' 
