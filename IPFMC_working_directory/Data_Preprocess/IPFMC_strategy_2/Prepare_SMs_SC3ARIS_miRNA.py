...
for Cancer_type in Cancer_type_list:
    Data_type = 'miRNA'  # Define the type of omics data to be used
    ...
    # Rename columns to replace '.' with '-' for consistency
    ...
    miRNASymbols = []  # Initialize a list to hold miRNA symbols for each pathway

    # Loop through each pathway to extract miRNA symbols
    for i in Pathways:
    ...
    miRNA_list = list(Omic_data.columns)  # Get all miRNA names from the omics data
    Valid_Pathways = []  # List to store valid pathways with sufficient miRNAs
    Valid_miRNASymbols = []  # List to store miRNA symbols corresponding to valid pathways

    # Filter pathways to retain only those with more than one miRNA present in the omics data
    for i in range(0, len(miRNASymbols)):
        SameGene = set(miRNA_list) & set(miRNASymbols[i])  # Find common miRNAs between omics data and current pathway
        SameGene = list(SameGene)  # Convert the set to a list
        Gene_Num = len(SameGene)  # Count the number of common miRNAs
        Patial_Omic = Omic_data.loc[:, SameGene]  # Subset the omics data to include only the common miRNAs
        if Gene_Num > 1:  # Check if there are more than one common miRNA
            Valid_Pathways.append(Pathways[i])  # Add the pathway to valid pathways
            Valid_miRNASymbols.append(miRNASymbols[i])  # Add the corresponding miRNA symbols
    ...
    # Loop through each set of significant miRNA symbols
    ...
    for i in range(6):  # Perform 6 iterations
        ...
        print(f'Current length: {Nums}')  # Print the current number of pathways
